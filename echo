...
-- RESCRIPTED BY SYNC>_<
local Players = game:GetService("Players")
local UserInputService = game:GetService("UserInputService")
local RunService = game:GetService("RunService")
local Camera = workspace.CurrentCamera
local Toggle = getgenv().DNS.Key360.Toggle
local RotationSpeed = getgenv().EC.Key360.RotationSpeed
local Keybind = getgenv().EC.Key360.Keybind
...
-- // Shorthand
local uwuEC = getgenv().EC
local uwuMain = uwuEC.General
local uwuCamMain = uwuDNS.Camlock.Main
local uwuCamFOV = uwuEC.Camlock.FOV
local uwuSilentMain = uwuEC.Silent.Main
local uwuSilentFOV = uwuEC.Silent.FOV
local uwuTrace = uwuEC.Tracer
...
local function SendNotification(text)
    Notification:Notify(
        {Title = "ECHO Z", Description = "IMPERIAL - "..text},
...
if uwuMain.Notifications then
    ...
    wait(3.5)
    SendNotification("INJECTED ECHO")
end
